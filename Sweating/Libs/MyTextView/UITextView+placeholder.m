...
- (UILabel *)label{
    ...
    if (label == nil) {
        ...
        label.textColor = [MyController colorWithHexString:@"d3d3d8"];
...
- (void)layoutSubviews
{
    ...
    self.label.y = TOP_MARGIN;
    self.label.x=LEFT_MARGIN;//设置 UILabel 的 x 值
    self.label.width=self.width-self.label.x*2.0; //设置 UILabel 的 x
    
    //根据文字计算高度
    
    CGSize maxSize =CGSizeMake(self.label.width,MAXFLOAT);
    
    self.label.height= [self.placeholder boundingRectWithSize:maxSize options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : self.label.font} context:nil].size.height;
}
//计算frame
