...
-(void)setTagWithTagArray:(NSArray*)arr{
    ...
    [arr enumerateObjectsUsingBlock:^(NSString*str, NSUInteger idx, BOOL *stop) {
        ...
        tagBtn.titleLabel.font=[UIFont boldSystemFontOfSize:11];
        ...
        if (arr.count) {
            if (0 == idx) {
                [tagBtn setBackgroundColor:[MyController colorWithHexString:CNavBgColor]];
    ...
    if(_GBbackgroundColor){
        ...
        self.backgroundColor=[MyController colorWithHexString:CLineColor];
...
//            button.selected=!button.selected;
            ...
            _tempBtn.backgroundColor=[MyController colorWithHexString:CLineColor];
             button.selected=YES;
            _tempBtn=button;
    ...
    if(button.selected==YES){
        button.backgroundColor=[MyController colorWithHexString:CNavBgColor];//[UIColor orangeColor];
        [button setTitleColor:R_G_B_16(0x818181) forState:UIControlStateNormal];
    }else if (button.selected==NO){
        button.backgroundColor=[UIColor clearColor];//[UIColor whiteColor];
    ...
    for(UIView*view in self.subviews){
        ...
        if([view isKindOfClass:[UIButton class]]){
            ...
            if (tempBtn.selected==YES) {
                ...
                tempBtn.backgroundColor=[MyController colorWithHexString:CNavBgColor];//[UIColor orangeColor];
                [tempBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
                
            }else {
                tempBtn.backgroundColor=[UIColor clearColor];//[UIColor whiteColor];
                [tempBtn setTitleColor:R_G_B_16(0x818181) forState:UIControlStateNormal];
